...
- 4.6.1
  - **429 Retry Handling Fix**: `RetryableContainer.HandleTooManyRequestsAsync` now applies exponential backoff to 429 TooManyRequests responses using a 3x multiplier per attempt and enforces a minimum delay floor of 100ms (instead of just taking whatever Cosmos suggested down to 1ms). Retry log messages now also include the failed request's `RequestCharge` (RUs). To support this, `RetryOptions.GetDelayForAttempt` gained two optional parameters (`delay`, `delayMultiplier`) that allow callers to override the instance defaults per-call without mutating shared `RetryOptions`.
