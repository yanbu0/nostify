...
1. **429 TooManyRequests**: Always retries. Waits for a delay derived from the server-provided `RetryAfter` header and then applies exponential backoff via `RetryOptions.GetDelayForAttempt(attempt, delay, delayMultiplier: 3)`. If Cosmos provides a positive `RetryAfter`, the effective base delay is the greater of that value and `Options.Delay.TotalMilliseconds`; if `RetryAfter` is missing or non-positive, the fallback floor of `MIN_DELAY_MS = 100ms` is used as the base delay. The current `RequestCharge` is included in the retry log message. Throws after `MaxRetries` exhausted.
   > **Important**: `ContainerExtensions.ApplyAndPersistAsync` — which is called internally by `RetryableContainer.ApplyAndPersistAsync` — must not catch or wrap 429 `CosmosException`s. The create-path catch and the patch outer-catch each explicitly rethrow 429 before their general `CosmosException` handler so the exception propagates back to `ExecuteWithRetryAsync`'s dedicated 429 clause. `SafePatchItemAsync` preserves its original "always returns a result" contract and does not rethrow 429; the outer `ApplyAndPersistAsync` catch block is responsible for propagating 429s that originate from the patch path.
