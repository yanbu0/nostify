...
1. **429 TooManyRequests**: Always retries. Waits for a delay derived from the server-provided `RetryAfter` header (with a minimum floor of `MIN_DELAY_MS = 100ms`) and then applies exponential backoff via `RetryOptions.GetDelayForAttempt(attempt, delay, delayMultiplier: 3)`. When the server omits `RetryAfter` (or returns a value below the floor), the floor of 100ms is used as the base delay. The current `RequestCharge` is included in the retry log message. Throws after `MaxRetries` exhausted.
