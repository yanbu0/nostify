...
- **4.6.2** - Fixed 409 Conflict errors in bulk create operations: `RetryableContainer.CreateItemAsync` now treats 409 on retry as idempotent success; `ApplyAndPersistAsync` (isNew path) catches 409 for at-least-once delivery; `BulkPersistEventAsync` awaits `HandleUndeliverableAsync`; template `BulkCreate_ReplaceMe_.cs` corrected to use `BulkCreate` command. Previously: Added unit tests for `RetryOptions.GetDelayForAttempt` per-call overrides; updated RetryOptions and RetryableContainer spec docs to reflect 4.6.2 429 retry behavior (3x exponential backoff with 100ms minimum floor, optional `delay` and `delayMultiplier` overrides)
