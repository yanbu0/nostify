...
- **4.6.1** - Added unit tests for `RetryOptions.GetDelayForAttempt` per-call overrides; updated RetryOptions and RetryableContainer spec docs to reflect 4.6.0 429 retry behavior (3x exponential backoff with 100ms minimum floor, optional `delay` and `delayMultiplier` overrides)
