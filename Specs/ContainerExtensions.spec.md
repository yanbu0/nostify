...
**Create path** (new item): Calls `Container.CreateItemAsync<T>`. Catches `409 Conflict` as idempotent success (at-least-once Kafka delivery). All other `CosmosException`s are wrapped in `NostifyException`. `429 TooManyRequests` is explicitly rethrown as a raw `CosmosException` so `RetryableContainer.ExecuteWithRetryAsync` can retry it.

**Patch path** (existing item): Calls `SafePatchItemAsync<T>` to build and apply diff-based patch operations. If `SafePatchItemAsync` returns a `PatchItemResult` with `statusCode == TooManyRequests`, the result is re-thrown as a raw `CosmosException(429)`. The outer `catch (CosmosException … TooManyRequests)` then propagates it to the retry layer. All other `CosmosException`s are wrapped in `NostifyException`.
