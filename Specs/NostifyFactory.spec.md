...
1. **Scan for EventType definitions** - Finds all concrete `EventType` subclasses in the assembly of `T` and creates a Kafka topic for each distinct event type name, preferring discoverable static `EventType` metadata when present and otherwise falling back to the concrete type name (which matches the template convention).
