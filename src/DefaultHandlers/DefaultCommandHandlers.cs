...
public static class DefaultCommandHandler
{
    ...
    {
        List<string> idStrings = JsonConvert.DeserializeObject<List<string>>(await new StreamReader(req.Body).ReadToEndAsync()) 
            ?? throw new NostifyException($"Failed to deserialize request body to list of IDs.");
        List<IEvent> peList = new List<IEvent>();
