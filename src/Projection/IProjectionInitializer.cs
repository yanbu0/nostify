...
public interface IProjectionInitializer
{
    ...
    /// </summary>
    Task<List<P>> InitAsync<P, A>(Guid id, INostify nostify, HttpClient? httpClient = null)
        where A : IAggregate
    ...
    /// </summary>
    Task<List<P>> InitAsync<P, A>(List<Guid> idsToInit, INostify nostify, HttpClient? httpClient = null)
        where A : IAggregate
        where P : NostifyObject, IProjection, IHasExternalData<P>, new();
    ...
    /// </summary>
    Task<List<P>> InitAsync<P>(List<P> projectionsToInit, INostify nostify, HttpClient? httpClient = null)
        where P : NostifyObject, IProjection, IHasExternalData<P>, new();
    ...
    /// </summary>
    Task InitContainerAsync<P, A>(INostify nostify, HttpClient? httpClient = null, string partitionKeyPath = "/tenantId", int loopSize = 1000)
        where A : IAggregate
        where P : NostifyObject, IProjection, IHasExternalData<P>, new();
    ...
    /// </summary>
    Task InitAllUninitialized<P>(INostify nostify, HttpClient? httpClient = null, int maxloopSize = 10)
        where P : NostifyObject, IProjection, IHasExternalData<P>, new();
}
